...
{#if $deleteProgressModal}
        ...
        <div class="fixed z-30 w-full h-full flex justify-center items-center">
            <div class="bg-white border border-gray-300 rounded-md  shadow-gray-400 shadow-md w-7/12 md:w-6/12 lg:w-5/12">
                ...
                {#if $deleteProgressBarModal}
                    <p class="m-4 text-black">Please wait while the schedules are being removed.</p>
                {:else if $deleteProgressBarFinished}
                    <p class="m-4 text-black">Your schedules have been removed Sucessfully!</p>
                {/if}
